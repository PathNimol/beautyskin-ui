    ...
    <DashboardLayout title="Suppliers" subtitle={`${MOCK_SUPPLIERS.length} registered suppliers`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map(supplier => (
          <div key={supplier.id} onClick={() => setSelectedSupplier(supplier)} className="bg-card border border-border rounded-2xl p-5 shadow-card hover:shadow-rose transition-all cursor-pointer hover:-translate-y-0.5">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1 mt-1">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-lg ${supplier.status === 'active' ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'}`}>{supplier.status}</span>
          ...
          <div className="relative bg-card border border-border rounded-2xl p-6 w-full max-w-md shadow-xl">
            ...
            <div className="flex items-center gap-4 mb-5">
              ...
              <div>
                ...
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-lg mt-1 inline-block ${selectedSupplier.status === 'active' ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'}`}>{selectedSupplier.status}</span>
              </div>
            </div>
