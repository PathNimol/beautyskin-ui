...
const SORT_OPTIONS = ['Featured', 'Price: Low to High', 'Price: High to Low', 'Best Rated', 'Newest'] as const;
...
export default function ProductListingClient({ embedded = false }: { embedded?: boolean }) {
  ...
  const catalogParams = useMemo(
    ...
    [searchQuery, activeCategory, priceRange.min, priceRange.max, activeSort]
  );
  ...
  const productDetailBase = '/product-detail';
  ...
  const hasActiveFilters =
    activeCategory !== 'All' || activePriceRange !== 0 || searchQuery.length > 0;
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center">
          ...
          <div className="flex shrink-0 items-center gap-3">
            <select
              ...
              className="min-h-[48px] cursor-pointer rounded-2xl border border-border bg-card px-4 py-3 text-sm text-foreground shadow-card focus:border-primary focus:outline-none"
            >
              {SORT_OPTIONS.map((o) => (
            ...
            <button
              ...
              className="flex min-h-[48px] items-center gap-2 rounded-2xl border border-border bg-card px-4 py-3 text-sm font-semibold text-foreground shadow-card md:hidden"
            >
        ...
        <div className="flex gap-8">
          <aside className="hidden w-60 shrink-0 md:block">
            <div className="sticky top-24 space-y-6 rounded-2xl border border-border bg-card p-5 shadow-card">
              <div>
            ...
            <div className="fixed inset-0 z-[100] md:hidden">
              ...
              <div className="absolute bottom-0 left-0 right-0 max-h-[85vh] overflow-y-auto rounded-t-3xl bg-card p-6">
                <div className="mb-6 flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => setSidebarOpen(false)} className="rounded-lg p-2 hover:bg-secondary">
                    ...
                    {CATEGORY_OPTIONS.map((cat) => (
                      <button
                        ...
                        className={`rounded-full px-4 py-2 text-sm font-medium ${
                          activeCategory === cat ? 'bg-primary text-foreground' : 'bg-secondary text-muted-foreground'
                        }`}
                      >
                        {cat}
                      </button>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {PRICE_RANGES.map((pr, idx) => (
                      <button
                        ...
                        className={`rounded-full px-4 py-2 text-sm font-medium ${
                          activePriceRange === idx ? 'bg-accent/20 text-gold-deep' : 'bg-secondary text-muted-foreground'
                        }`}
                      >
                        {pr.label}
                      </button>
            ...
            <div className="mb-5 flex items-end justify-between gap-4">
              <div>
                <p className="text-sm text-muted-foreground">
                  <span className="text-lg font-extrabold text-foreground">{totalElements}</span> products
                  {totalPages > 1 && (
                    <span>
                      {' '}
                  ...
                  {products.map((product) => (
                    ...
                    >
                      <div className="relative h-56 overflow-hidden bg-secondary">
                        ...
                        {!product.inStock && (
                          <div className="absolute inset-0 flex items-center justify-center bg-foreground/50 backdrop-blur-[2px]">
                            <span className="rounded-lg bg-card px-3 py-1.5 text-xs font-bold">Out of stock</span>
                          </div>
                        )}
                      </div>
                      <div className="flex flex-1 flex-col p-4">
                        ...
                        {product.shopName && (
                          <p className="mb-2 text-[10px] text-muted-foreground">{product.shopName}</p>
                        )}
