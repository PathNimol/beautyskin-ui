    ...
    <DashboardLayout title="Reports" subtitle="Analytics and business insights">
      ...
      <div className="flex items-center gap-2 mb-6">
        {(['daily', 'weekly', 'monthly', 'yearly'] as Period[]).map(p => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all capitalize ${
              period === p ? 'bg-primary text-foreground shadow-rose' : 'bg-card border border-border text-muted-foreground hover:text-foreground'
            }`}
          >
            {p}
          </button>
        ...
        <div className="ml-auto flex gap-2">
          <button
            onClick={() => exportCSV('revenue-report.csv', ['Month', 'Revenue', 'Orders'], REVENUE_DATA.map(d => [d.month, d.revenue, d.orders]))}
            className="flex items-center gap-2 px-4 py-2 bg-green-50 border border-green-200 text-green-700 text-sm font-semibold rounded-xl hover:bg-green-100 transition-all"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-red-50 border border-red-200 text-red-600 text-sm font-semibold rounded-xl hover:bg-red-100 transition-all"
          >
        ...
        <div className="bg-card border border-border rounded-2xl p-6 shadow-card">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Monthly revenue over the year</p>
            </div>
          </div>
      ...
      <div className="bg-card border border-border rounded-2xl shadow-card overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5 border-b border-border">
          ...
          <button
            onClick={() => exportCSV('top-products.csv', ['Product', 'Brand', 'Units Sold', 'Revenue', 'Rating'], topProducts.map(p => [p.name, p.brand, p.sold, p.price * p.sold, p.rating]))}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-green-50 border border-green-200 text-green-700 text-xs font-semibold rounded-lg hover:bg-green-100 transition-all"
          >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-secondary/30">
                <th className="text-left px-6 py-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Rank</th>
                <th className="text-left px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Product</th>
                <th className="text-right px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Units Sold</th>
                <th className="text-right px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Revenue</th>
                <th className="text-center px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Rating</th>
                <th className="text-left px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Performance</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={product.id} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                    <td className="px-6 py-4">
                      <div className={`w-7 h-7 rounded-xl flex items-center justify-center text-xs font-extrabold ${
                        i === 0 ? 'bg-amber-100 text-amber-700' : i === 1 ? 'bg-gray-100 text-gray-600' : i === 2 ? 'bg-orange-100 text-orange-700' : 'bg-secondary text-muted-foreground'
                      }`}>
                        {i + 1}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4 text-right">
                      <span className="text-sm font-bold text-foreground">{product.sold.toLocaleString()}</span>
                    </td>
                    <td className="px-4 py-4 text-right">
