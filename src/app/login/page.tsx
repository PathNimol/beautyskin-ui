...
const ROLE_HINTS = [
  { role: 'Super Admin', email: 'admin@beautyskin.com', password: 'admin123', color: 'bg-purple-50 text-purple-700 border-purple-200', redirect: '/admin/dashboard' },
  { role: 'Shop Owner', email: 'owner@beautyskin.com', password: 'owner123', color: 'bg-rose-50 text-rose-700 border-rose-200', redirect: '/owner/dashboard' },
  { role: 'Staff', email: 'staff@beautyskin.com', password: 'staff123', color: 'bg-blue-50 text-blue-700 border-blue-200', redirect: '/staff/dashboard' },
  { role: 'Customer', email: 'buyer@beautyskin.com', password: 'buyer123', color: 'bg-green-50 text-green-700 border-green-200', redirect: '/customer/account' },
];

export default function LoginPage() {
  const router = useRouter();
  const { signIn, role: currentRole, isAuthenticated } = useMockAuth();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(hint?.redirect ?? '/admin/dashboard');
    } catch {
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <Link href="/forgot-password" className="text-xs text-accent hover:text-gold-deep transition-colors font-medium">
                  Forgot password?
                </Link>
              </div>
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            ...
            <Link href="/register" className="text-accent hover:text-gold-deep font-semibold transition-colors">
              Create account
            </Link>
          </p>
          ...
          <div className="mt-8 p-5 bg-secondary/60 rounded-2xl border border-border">
            <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3">Demo Credentials</p>
            ...
            <p className="text-[10px] text-muted-foreground mt-3 text-center">Click a role to auto-fill credentials</p>
          </div>
        </div>
      </div>
    </div>
